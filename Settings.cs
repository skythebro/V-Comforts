...
namespace VrisingQoL
{
	public static class Settings
	{
		...
		internal static void Initialize(ConfigFile config)
		{
			...
			ENABLE_PREDEFINED_RESPAWN_POINTS = config.Bind<bool>("Respawn", "enablePredefinedRespawnPoints", true, "If enabled a json file in 'BepInEx\\config\\Respawns\\respawns.json' can be created with a list of coordinates where the respawnpoints will be placed, each player will then get ownership of each instance of a respawnpoint at those locations (if a player logs in another instance of the respawnpoint will be created at the same location and vice versa when logging out).");
			...
			RESPAWN_POINT_COST_AMOUNT = config.Bind<int>("Respawn", "enableRespawnPointCostAmount", 1, "If above 0, respawn points will cost X amount of prefabitem defined below. (if 0 it will not cost anything)");
			RESPAWN_POINT_COST_ITEM_PREFAB = config.Bind<int>("Respawn", "enableRespawnPointCostItemPrefab", 271594022, "Sets the item required in the users inventory to spawn a respawnpoint. (this is the prefab item id)");
